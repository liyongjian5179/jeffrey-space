...
- `vite.config.js`：Vite 构建配置，生产 bundle 输出到 `dist/_assets`，静态资源保留在 `dist/assets`。
...
- `vite.config.js`: Vite build configuration. Production bundles go to `dist/_assets`, while static site assets stay under `dist/assets`.
