...
- `index.html`：静态入口，包含统计脚本、导航、页面结构、弹窗和资源加载。
- `src/content.js`：中英双语内容数据，包括导航、首页、观点、热爱、经历、技能、输出和联系方式。
- `src/app.js`：页面渲染、语言切换、滚动 reveal、项目图库、二维码弹窗等交互逻辑。
...
- `index.html`: Static entry file, analytics, navigation, page sections, modals, and script/style loading.
- `src/content.js`: Structured bilingual content for navigation, hero, beliefs, projects, experience, skills, outputs, and contact.
- `src/app.js`: Rendering, language switching, reveal animation, project gallery, QR modal, and interactions.
...
Run lightweight syntax checks before publishing:
